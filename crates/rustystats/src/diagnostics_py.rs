...
use rustystats_core::diagnostics::{
    aic, bic, compute_ae_categorical, compute_ae_continuous, compute_calibration_curve,
    compute_discrimination_stats, compute_family_loss, compute_lorenz_curve,
    compute_residual_pattern_continuous, detect_interactions, hosmer_lemeshow_test, mae, mse,
    null_deviance, pearson_chi2, resid_deviance, resid_pearson, rmse, FactorData,
    InteractionConfig,
};
...
pub fn compute_fit_statistics_py<'py>(
    ...
    let _dispersion_deviance = if df_resid > 0 {
        deviance / df_resid as f64
    } else {
        1.0
    };
    let dispersion_pearson = if df_resid > 0 {
...
pub fn compute_dataset_metrics_py<'py>(
    ...
    let parsed_theta = if family_lower.starts_with("negativebinomial")
