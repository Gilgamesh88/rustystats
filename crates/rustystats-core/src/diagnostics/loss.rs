...
pub fn poisson_deviance_loss(
    ...
    match weights {
        Some(w) => {
            ...
            let weighted_sum: f64 = unit_deviances
                .iter()
                .zip(w.iter())
                .map(|(&d, &wi)| wi * d)
                .sum();
        ...
        None => unit_deviances.iter().sum::<f64>() / n as f64,
    }
}
...
pub fn log_loss(y: &Array1<f64>, mu: &Array1<f64>, weights: Option<&Array1<f64>>) -> f64 {
    ...
    match weights {
        Some(w) => {
            ...
            let weighted_sum: f64 = unit_losses
                .iter()
                .zip(w.iter())
                .map(|(&l, &wi)| wi * l)
                .sum();
        ...
        None => unit_losses.iter().sum::<f64>() / n as f64,
    }
}
