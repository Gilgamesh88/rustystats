...
from .constants import DEFAULT_N_PERMUTATIONS, DEFAULT_PRIOR_WEIGHT
...
def apply_target_encoding(
    ...
    return _rustystats.apply_target_encoding_py(categories, level_stats, prior, prior_weight)
...
class TargetEncoder:
    ...
    def __init__(
        ...
        self.level_stats_: dict[str, tuple[float, int]] | None = None
    ...
    def fit(
        ...
        _, _, self.prior_, self.level_stats_ = target_encode(
            categories, target, "x", self.prior_weight, self.n_permutations, self.seed
        )
        return self
    ...
    def transform(
        ...
        return apply_target_encoding(categories, self.level_stats_, self.prior_, self.prior_weight)
...
class FrequencyEncoder:
    ...
    def __init__(self):
        self.level_counts_: dict[str, int] | None = None
        self.max_count_: int | None = None
...
def target_encode_interaction(
    cat1: list[str] | np.ndarray,
    cat2: list[str] | np.ndarray,
    target: np.ndarray,
    var_name1: str = "x1",
    var_name2: str = "x2",
    prior_weight: float = DEFAULT_PRIOR_WEIGHT,
    ...
    return _rustystats.target_encode_interaction_py(
        cat1, cat2, target, var_name1, var_name2, prior_weight, n_permutations, seed
    )
