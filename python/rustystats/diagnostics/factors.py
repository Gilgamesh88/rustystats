...
class _FactorDiagnosticsComputer:
    ...
    def compute_factor_diagnostics(
        ...
        can_compute_score_test = (
            design_matrix is not None and bread_matrix is not None and irls_weights is not None
        )
        ...
        for name in categorical_factors:
            ...
            significance = (
                self.compute_factor_significance(name, result, bread_matrix)
    ...
    def _get_factor_coefficients(self, name: str, result) -> list[FactorCoefficient | None]:
